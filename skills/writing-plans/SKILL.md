...
4. **Standards (`karpathy-guidelines`)** — every task has a verifiable check (a test/command that proves it works); no task plans speculative flexibility, single-use abstractions, or code that's bigger than it needs to be.
