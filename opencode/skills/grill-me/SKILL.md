...
- Building a design from a vague idea (not stress-testing an existing one) -> use `formal-design` only when the user explicitly requests formal design or approved material-risk criteria apply; otherwise use direct execution.
- Once the design survives grilling -> use `formal-planning` only when the user explicitly requests a plan or materially risky multi-stage work needs dependencies and coordinated checkpoints; otherwise use direct execution. Do not hand off automatically.
